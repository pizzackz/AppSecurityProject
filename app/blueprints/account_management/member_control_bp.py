...
from app import db
from app.models import User, Member, LockedAccount
from app.forms.forms import LockDeleteMemberForm
from app.utils import clean_input, generate_otp, send_email, check_auth_stage, check_jwt_values, check_admin, check_session_keys, clear_unwanted_session_keys, get_image_url, upload_pfp, reset_pfp
...
def view_member_details():
    ...
    if action:
        ...
        if action == "revoke_plan":
            ...
            return redirect(url_for("member_control_bp.revoke_member_plan"))
...
def lock_member():
    ...
    # Redirect to view ,e,ber details & clear temp data in session & jwt when pressed 'back'
